//! The tile source: worker threads fetching texture + heightmap + normals per
//! tile (disk cache or HTTP with atomic write-through), decoding PNGs,
//! synthesizing heightmaps above the native zoom, and delivering whole-tile
//! payloads through channels drained once per frame.
//!
//! Deliberately plain `std::thread` + crossbeam (NOT bevy_tasks): HTTP here is
//! blocking, and the raytiles pool semantics — dedup by key, cancel flags
...
impl Worker {
    ...
    fn fetch_heightmap(&self, agent: &ureq::Agent, req: &TileRequest) -> Result<RgbImage, String> {
        ...
        for level in (native + 1)..=req.key.zoom {
            let shift = req.key.zoom - level;
            let qx = ((req.x >> shift) & 1) as usize;
            let qz = ((req.z >> shift) & 1) as usize;
            floats = synth::upsample_quadrant(&floats, w, h, qx, qz);
        }
        let out = synth::encode_terrarium(&floats, w as u32, h as u32);
    ...
    fn url_template(&self, kind: &Kind) -> &str {
        match kind {
            Kind::Texture => &self.cfg.texture_url,
            Kind::Heightmap => &self.cfg.heightmap_url,
            Kind::Normals => &self.cfg.normals_url,
        }
    }
}
...
fn expand_url(template: &str, zoom: u8, x: i32, z: i32) -> String {
    template
